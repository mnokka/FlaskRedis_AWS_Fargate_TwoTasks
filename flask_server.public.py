...
# 2 dockers in local machine
#REDIS_HOST = os.getenv('REDIS_HOST', ',my_redis_2')  
#REDIS_PORT = os.getenv('REDIS_PORT', 6379)

REDIS_HOST = 'redis-service.toka-namespace.local'
REDIS_PORT = 6379
...
def get_redis_address():
    ...
    logging.info(f"*** boto3 session:{session} ***")

    ecs = boto3.client("ecs", region_name="eu-north-1")
    cluster_name = "arn:aws:ecs:eu-north-1:<YOUR_AWS_ID>:cluster/my-cluster"
    service_name = "redis-service"

    logging.info(f"*** ecs:{ecs} ***")

    response = ecs.list_clusters()
    logging.info("RESPONSE *******************************")
    logging.info(response['clusterArns'])
   
    credentials = session.get_credentials()

    if credentials is None:
        logging.info("** NO CREDNTIALS FOUND****")
    else:
        ...
        for attachment in task.get("attachments", []):
            for detail in attachment.get("details", []):
                if detail["name"] == "privateIPv4Address":
                    return detail["value"]  # Redis-taskin osoite
...
def check_redis_connection(redis_address):
...
result=get_redis_address()
logging.info(f"GOT REDIS ADDRESS BACK:{result}")

check_redis_connection(result)
logging.info(f"Carry on booting...")
...
def create_competitor():
    ...
    http://localhost:5001/competitor
    """
    ...
    if missing_keys:
        ...
        return jsonify({"error": f"Missing fields: {', '.join(missing_keys)}"}), 555
    ...
    if extra_keys:
        ...
        return jsonify({"error": f"Extra fields: {', '.join(extra_keys)}"}), 555

    if data['gender'] not in ALLOWED_GENDERS:
        logging.error(f"Competitor Gender error: {data['gender']}")
        return jsonify({"error": f"Gender error: {data['gender']}"}), 555

    if data['category'] not in ALLOWED_CATEGORIES:
        logging.error(f"Competitor Category error: {data['category']}")
        return jsonify({"error": f"Category error: {data['category']}"}), 555
    ...
    try:
        ...
        return jsonify({"error": "Missing 'bib' key in data"}), 555
...
curl -X POST http://localhost:5001/settimes \
-H "Content-Type: application/json" \
-d '{
      "bib": 123,
      "settimes": {
    ...
    if not bib or not settimes:
        ...
        return jsonify({"error": "Missing data"}), 555
    ...
    if missing_checkpoints:
        ...
        return jsonify({"error": f"Missing checkpoints: {', '.join(missing_checkpoints)}"}), 555
    ...
    if invalid_checkpoints:
        ...
        return jsonify({"error": "Invalid checkpoint(s) in request"}), 555
    ...
    for checkpoint, timestamp in settimes.items():
        if timestamp != -1:
            ...
            if milliseconds is None:
                return jsonify({"error": "Invalid time format. Use string hh:mm:ss:SSS"}), 555
...
def check_competitor(bib_number):
    ...
    curl -X GET http://localhost:5001/check_competitor/123
    ...
    if competitor_data['gender'] not in ALLOWED_GENDERS:
        ...
        return jsonify({"error": f"Eroro in gender: {competitor_data['gender']}"}), 555

    if competitor_data['category'] not in ALLOWED_CATEGORIES:
        logging.error(f"Error in category: {competitor_data['category']}")
        return jsonify({"error": f"Error in category: {competitor_data['category']}"}), 555
...
def check_checkpoints(bib_number):
    ...
    curl -X GET http://localhost:5001/checkpoints/123
    ...
    if invalid_checkpoints:
        ...
        return jsonify({"error": f"Illegal checkpoints: {', '.join(invalid_checkpoints)}"}), 555
